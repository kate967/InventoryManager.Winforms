...
namespace InventoryManager.Data
{
    public class Player : INotifyPropertyChanged
    {
        ...
        public void BuildEquippedItemsFromName(List<Item> items)
        {
            EquippedItems = (from entry in EquippedItemNames
                            ...
                            where item != null
                            select (EquipLocations: entry.Key, Item: item)).ToDictionary(pair => pair.EquipLocations, pair => pair.Item);
    ...
    public class PlayerConverter : JsonConverter
    {
        ...
        public override object ReadJson(JsonReader reader, Type objectType, object existingValue, JsonSerializer serializer)
        {
            ...
            if(jsonObject.TryGetValue("EquippedItems", out JToken equipedItemNamesToken))
            {
                equippedItemNames = equipedItemNamesToken.ToObject<Dictionary<EquipLocations, string>>();
            }
            else
            {
